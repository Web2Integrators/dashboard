...
export default component$(() => {
  ...
  const mydata  = useStore<MyState>({charts: []});
 
  const { document } = new JSDOM().window;
 // const chart = useSignal<SVGSVGElement | HTMLElement>();
 useTask$(() => {
  mydata.charts.push(Plot.plot({
    marks: [
      Plot.dot(dataset, {
        x: (d) => d.x,
        y: (d) => d.y,
  ...
  // chart.value = useSignal<SVGSVGElement | HTMLElement>(
    // Plot.plot({
    //   marks: [
    //     Plot.dot(dataset, {
    //       x: d => d.x,
  ...
  // Plot.lineY([1, 2, 0, 4, 0, 3, 1, 5, 7, 2, 3]).plot({axis: null, width: 80, height: 18,document}));
  //Plot.dot(data, {x: "culmen_length_mm", y: "culmen_depth_mm", fill: "species"}).plot({axis: null, width: 80, height: 18,document}));
  // Plot.dotX("abcdef", {r: 5, fill: Plot.identity}).plot({axis: null, width: 80, height: 12,document}));
  // Plot.plot({
  ...
  //         Anomaly: t.Anomaly,
  //       })),
  //       Plot.windowY(12, { x: "Date", y: "Anomaly" })
  //     ),
  //     Plot.ruleX([0], {
  //       stroke: "gray",
  //       strokeDasharray: "5,3",
  ...
  return (
    <>
      <div dangerouslySetInnerHTML={mydata.charts[0].outerHTML}></div>
      <div dangerouslySetInnerHTML={mydata.charts[1].outerHTML}></div>
    </>
  );
});
